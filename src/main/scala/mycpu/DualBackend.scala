...
class DualBackend extends Module {
    ...
    // MAX.WU 沿用 RRWINZ 的串行策略，进入 EX 时一定占据较老的 lane0。
    ...
    // 使用独立单元，保持随机自定义指令统一仿照 RRWINZ 的实现策略。
    ...
    for (lane <- 0 until 2) {
        ...
        // MAX.WU 选择专用单元结果；其他指令仍走原 MDU/普通结果路径。
    ...
    for (lane <- 0 until 2) {
        ...
        val src1 = inst(9, 5)
        // MAX.WU 使用常规 3R 读口：src1=rj，src2=rk。
        ...
        // 保存译码出的 MAX.WU 身份，防止它在 ID/IS 缓冲等待期间丢失。
        ...
        // MAX.WU 沿用 RRWINZ 策略，只允许作为 lane0 单发。
    ...
    for (lane <- 0 until 2) {
        ...
        // 将控制位装入 PipelineData，使 EX 及后续写回选择保持一致。
