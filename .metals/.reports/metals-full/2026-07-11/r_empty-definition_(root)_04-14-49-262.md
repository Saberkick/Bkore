error id: file://<WORKSPACE>/src/main/scala/mycpu/Cache.scala:
file://<WORKSPACE>/src/main/scala/mycpu/Cache.scala
empty definition using pc, found symbol in pc: 
empty definition using semanticdb
empty definition using fallback
non-local guesses:
	 -chisel3/current_index.
	 -chisel3/current_index#
	 -chisel3/current_index().
	 -chisel3/util/current_index.
	 -chisel3/util/current_index#
	 -chisel3/util/current_index().
	 -current_index.
	 -current_index#
	 -current_index().
	 -scala/Predef.current_index.
	 -scala/Predef.current_index#
	 -scala/Predef.current_index().
offset: 10956
...
class Cache extends Module {
    ...
    //If new addr (Both Idx and Offset) is the same as current write addr, the inst has to be stalled
    val hazard_with_lookup = is_lookup_write && (io.cpu.index === req_index) && (io.cpu.offset(3,2) === req_offset(3,2))
    val hazard_with_wb     = wb_state === wbWrite && (io.cpu.index === wb_index) && (io.cpu.offset(3,2) === req_offset(3,2))
    ...
    val current_offset = Mux(main_state === sIdle || main_state === sLookup, io.cpu.offset, req_offset)
    // 【核心魔法】：精准读使能控制
        // 1. 只有在 Idle(闲置) 或 Lookup(查表) 时，我们才去读目标 Bank。
        // 2. 如果发生了 Miss，为了把一整行踢掉，必须在 sMiss 阶段将 4 个 Bank 全读出来。
        // 3. 在 Refill 阶段绝不读 RAM，防止与 AXI 的写入发生冲突！
    val is_reading_state = (main_state === sIdle) || (main_state === sLookup)
    val is_miss_state    = (main_state === sMiss)
    val ren_b0 = is_miss_state || (is_reading_state && current_offset(3,2) === 0.U)
    val ren_b1 = is_miss_state || (is_reading_state && current_offset(3,2) === 1.U)
    val ren_b2 = is_miss_state || (is_reading_state && current_offset(3,2) === 2.U)
    val ren_b3 = is_miss_state || (is_reading_state && current_offset(3,2) === 3.U)

    // 调用 Chisel 的 read(addr, enable) 接口
    val rdata_w0_b0 = data_way0_bank0.read(curr_idx, ren_b0)
    val rdata_w0_b1 = data_way0_bank1.read(current_index@@, ren_b1)
    val rdata_w0_b2 = data_way0_bank2.read(current_index, ren_b2)
    val rdata_w0_b3 = data_way0_bank3.read(current_index, ren_b3)

    val rdata_w1_b0 = data_way1_bank0.read(current_index, ren_b0)
    val rdata_w1_b1 = data_way1_bank1.read(current_index, ren_b1)
    val rdata_w1_b2 = data_way1_bank2.read(current_index, ren_b2)
    val rdata_w1_b3 = data_way1_bank3.read(current_index, ren_b3)
