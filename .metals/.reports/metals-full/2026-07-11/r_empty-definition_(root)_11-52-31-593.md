error id: file://<WORKSPACE>/src/main/scala/mycpu/Top.scala:inst_uncached
file://<WORKSPACE>/src/main/scala/mycpu/Top.scala
empty definition using pc, found symbol in pc: inst_uncached
empty definition using semanticdb
empty definition using fallback
non-local guesses:
	 -chisel3/if_stage/io/inst_uncached.
	 -chisel3/if_stage/io/inst_uncached#
	 -chisel3/if_stage/io/inst_uncached().
	 -chisel3/util/if_stage/io/inst_uncached.
	 -chisel3/util/if_stage/io/inst_uncached#
	 -chisel3/util/if_stage/io/inst_uncached().
	 -if_stage/io/inst_uncached.
	 -if_stage/io/inst_uncached#
	 -if_stage/io/inst_uncached().
	 -scala/Predef.if_stage.io.inst_uncached.
	 -scala/Predef.if_stage.io.inst_uncached#
	 -scala/Predef.if_stage.io.inst_uncached().
offset: 8533
...
class mycpu_top extends RawModule {
    ...
    withClockAndReset(aclk, reset_high) {
        ...
        // ==========================================
        // -- 指令端 (IF) --
        // 1. CPU 流水线取指级连入 ICache 的 CPU 侧接口
        icache.io.cpu.valid  := if_stage.io.inst_sram.req
        icache.io.cpu.op     := false.B // ICache 恒为读 (0)
        icache.io.cpu.index  := if_stage.io.inst_sram.addr(11, 4)  // VIPT 切片
        icache.io.cpu.tag    := if_stage.io.inst_sram.addr(31, 12)
        icache.io.cpu.offset := if_stage.io.inst_sram.addr(3, 0)
        icache.io.cpu.wstrb  := 0.U
        icache.io.cpu.wdata  := 0.U
        icache.io.cpu.uncached := if_stage.io.inst_unc@@ached

        if_stage.io.inst_sram.addr_ok := icache.io.cpu.addr_ok
        if_stage.io.inst_sram.data_ok := icache.io.cpu.data_ok
        ...
        // -- 数据端 (EX & MEM) --
        dcache.io.cpu.valid  := ex_stage.io.data_sram.req
        ...
        ex_stage.io.data_sram.addr_ok := dcache.io.cpu.addr_ok
        ...
        // 3. 幽灵数据探头：现在 MEM 级应该去盯 DCache 的握手，而不是 Bridge 的握手
        mem_stage.io.sram_req         := ex_stage.io.data_sram.req
        mem_stage.io.sram_addr_ok     := dcache.io.cpu.addr_ok
        mem_stage.io.sram_data_ok     := dcache.io.cpu.data_ok
