error id: file://<WORKSPACE>/src/main/scala/mycpu/Top.scala:debug_wb_pc
file://<WORKSPACE>/src/main/scala/mycpu/Top.scala
empty definition using pc, found symbol in pc: debug_wb_pc
empty definition using semanticdb
empty definition using fallback
non-local guesses:
	 -chisel3/wb_stage/io/debug_wb_pc.
	 -chisel3/wb_stage/io/debug_wb_pc#
	 -chisel3/wb_stage/io/debug_wb_pc().
	 -chisel3/util/wb_stage/io/debug_wb_pc.
	 -chisel3/util/wb_stage/io/debug_wb_pc#
	 -chisel3/util/wb_stage/io/debug_wb_pc().
	 -wb_stage/io/debug_wb_pc.
	 -wb_stage/io/debug_wb_pc#
	 -wb_stage/io/debug_wb_pc().
	 -scala/Predef.wb_stage.io.debug_wb_pc.
	 -scala/Predef.wb_stage.io.debug_wb_pc#
	 -scala/Predef.wb_stage.io.debug_wb_pc().
offset: 4633
...
class mycpu_top extends RawModule {
    ...
    withClockAndReset(clk, reset_high) {
        ...
        inst_sram_req            := if_stage.io.inst_sram.req
        inst_sram_wr             := if_stage.io.inst_sram.wr
        inst_sram_size           := if_stage.io.inst_sram.size
        inst_sram_wstrb          := if_stage.io.inst_sram.wstrb
        inst_sram_addr           := if_stage.io.inst_sram.addr
        inst_sram_wdata          := if_stage.io.inst_sram.wdata
        if_stage.io.inst_sram.addr_ok := inst_sram_addr_ok
        if_stage.io.inst_sram.data_ok := inst_sram_data_ok
        if_stage.io.inst_sram.rdata   := inst_sram_rdata

        data_sram_req            := ex_stage.io.data_sram.req
        data_sram_wr             := ex_stage.io.data_sram.wr
        data_sram_size           := ex_stage.io.data_sram.size
        data_sram_wstrb          := ex_stage.io.data_sram.wstrb
        data_sram_addr           := ex_stage.io.data_sram.addr
        data_sram_wdata          := ex_stage.io.data_sram.wdata

        ex_stage.io.data_sram.addr_ok := data_sram_addr_ok
        ex_stage.io.data_sram.data_ok := data_sram_data_ok  //虽然 EX 级不用 data_ok，但补齐连线防止报错
        ex_stage.io.data_sram.rdata   := data_sram_rdata    //同上
        
        mem_stage.io.sram_data_ok     := data_sram_data_ok 
        mem_stage.io.sram_rdata       := data_sram_rdata

        // 9. Debug 接口连线
        debug_wb_pc       := wb_stage.io.debug@@_wb_pc
